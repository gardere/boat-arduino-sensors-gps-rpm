...
float fullVoltage = 3.32f; //Best result after calibration
...
unsigned int getSensorResistance(int pinNumber) {
  float rawReading = analogRead(pinNumber) / 1024.0f * fullVoltage; //aka Vout  in voltage divider equation - arduino analog inputs readings return a number of (5V / 1024 'steps')
